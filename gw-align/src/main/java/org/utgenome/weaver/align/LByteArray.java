...
/**
 * Bayte array capable to store more than 2G (2 x 1024 x 1024 x 1024) entries
 * 
 * @author leo
 * 
 */
public class LByteArray
{
    ...
    public LByteArray(long size) {
        ...
        array = new ArrayList<byte[]>(container + 1);
        for (int i = 0; i < container; i++) {
            array.add(new byte[Integer.MAX_VALUE]);
        }
        if (remainder > 0)
            array.add(new byte[remainder]);
