...
class ACGTNSeq(private val seq:Array[Long], val numBases:Long)
  ...
  override def equals(obj:Any) = {
    ...
    while (pos < filledBlockSize) {
      if (this.seq(pos) != other.seq(pos)) return false
      pos += 1;
      pos - 1
    }


    val offset: Int = (numBases % 64L).asInstanceOf[Int]


    if (offset > 0) {
      val mask: Long = new Array[Long](3)
      mask(0) = ~0L << 64 - offset
      ...
      mask(2) = if (offset <= 32) 0L else ~0L << (64 - offset) * 2
      {
        ...
        while (i < mask.length) {
          {
            if ((seq(pos + i) & mask(i)) != (other.seq(pos + i) & mask(i))) return false
          }
          ({
            i += 1; i - 1
          })
